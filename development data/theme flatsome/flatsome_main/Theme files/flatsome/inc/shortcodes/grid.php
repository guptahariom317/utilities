...
function rowShortcode($atts, $content = null) {
  ...
  ob_start();
  ?>
	<div class="row container<?php if($style) echo ' '.$style; ?><?php if($width){ ?> custom-width<?php } ?> <?php echo $css; ?>"<?php if($width){ ?>style="max-width:<?php echo $width; ?>"<?php } ?>><?php echo fixShortcode($content); ?></div>
  <?php
  $content = ob_get_contents();
  ob_end_clean();
  return $content;
...
function colShortcode($atts, $content = null) {	
  ...
  if($tooltip) {
	...
	$column = '<div class="small-'.$small.''.$align.''.$tooltip_class.' '.$class.' large-'.$span.' '.$hover.' columns '.$scroll.'" '.$tooltip.' '.$scroll_html.' '.$delay_html.'><div class="column-inner" '.$padding.'>'.$content.'</div></div>';
